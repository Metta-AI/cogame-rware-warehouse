...
image_tag="coworld-rware-warehouse-replay-viewer-build:$$"
container_id=""
cleanup() {
...
if docker buildx version >/dev/null 2>&1; then
  ...
  # explicit amd64 platform through their Linux VM. CI installs Buildx.
  docker build "${build_args[@]}"
fi
container_id="$(docker create --platform linux/amd64 "${image_tag}")"
docker cp "${container_id}:/workspace/rware-warehouse/replay-viewer/dist/." "${output_dir}"

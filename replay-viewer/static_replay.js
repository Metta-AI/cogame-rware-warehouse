(function () {
  ...
  function createCore(config) {
    ...
    var lastFrame = 0;
    var accumulator = 0;
    // The presentation cadence. The RUNTIME owns how many sim ticks a frame
    // advances (replay_runtime.TicksPerSecondBase against the speed chip), so
    // this is only how often the Worker is asked for one.
    var frameMs = 1000 / ((window.RWARE_WIRE && window.RWARE_WIRE.fps) || 30);
    var workerDraws = 0;
    ...
    function start() {
      ...
      try {
        worker = new Worker(workerUrl, { name: 'rware-static-replay' });
